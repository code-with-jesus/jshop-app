...
public class S3Service {
	...
	public String storeProfileImage(MultipartFile uploadedFile, String username) throws IOException {
		...
		if (uploadedFile != null && !uploadedFile.isEmpty()) {
			...
			File tmpImagesStoredFolder = new File(tempImageStore + File.separatorChar + username);
			if (!tmpImagesStoredFolder.exists()) {
				LOG.info("Creating the temporary root for the S3 assets");
				tmpImagesStoredFolder.mkdirs();
			}
			
			// The temporary file where the profile image will be stored
			File tmpProfileImageFile = new File(tmpImagesStoredFolder.getAbsolutePath()
					+ File.separatorChar
	...
	private String storeProfileImageToS3(File resource, String username) {
		...
		if (!resource.exists()) {
			LOG.error("The file {} does not exists. Throwing an exception", resource.getAbsolutePath());
		...
		if (null == rootBucketUrl) {
			LOG.error("The bucket {} does not exists and the application was not able to create it. THe image won't be stored with the profile", rootBucketUrl);
		} else {
			...
			try {
				...
				LOG.error("A client exception occurred while trying to store the profile  image {} on S3."
						+ " The profile image won't be stored", resource.getAbsolutePath(), ace);
			}
		}
		return resourceUrl;
	}
	...
	private String ensureBucketExists(String bucketName) {
		...
		try {
			...
			LOG.error("An error occurred while connection to S3. Will not execute action for bucket: {}", bucketName, ace);
		}
		return bucketUrl;
	}
}
